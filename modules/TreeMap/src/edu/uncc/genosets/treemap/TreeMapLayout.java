...
public class TreeMapLayout extends TreeLayout{
    ...
    public DAGGraph backingGraph;
    public String backingAreaColumn;
    public String sourceIndexField = "source_node";
    ...
    //column name to base area on
    public static String areaColumn = "count";

    static {
        AREA_SCHEMA.addColumn(AREA, double.class);
    }

    static Comparator s_cmp = new Comparator() {
    ...
    ArrayList m_kids = new ArrayList();
    ArrayList m_row  = new ArrayList();
    Rectangle2D m_r  = new Rectangle2D.Double();

    double m_frame; // space between parents border and children
    /**
     * Creates a new AuroraTreeMapLayout with no spacing between
     * parent areas and their enclosed children.
     * @param group the data group to layout. Must resolve to a Graph instance.
     */
    public TreeMapLayout(Graph backingGraph, String group, String areaColumn) {
        this(group, 0);
        this.backingGraph = (DAGGraph)backingGraph;
        this.backingAreaColumn = areaColumn;
    }

    /**
     * Creates a new AuroraTreeMapLayout with the specified spacing between
     * parent areas and their enclosed children.
    ...
    public void setAreaColumn(String backingColumnName){
        this.backingAreaColumn = backingColumnName;
    }
    ...
    private void setNodeDimensions(NodeItem n, double w, double h) {
        ...
        /**
     * Compute the pixel areas of nodes based on their areaColumn values.
     * and stores in
     */
    protected void computeAreas(NodeItem root){
        int leafCount = 0;
...
//        Iterator nIter = nodes.tuples();
//        while(nIter.hasNext()){
//            NodeItem n = (NodeItem)nIter.next();
//            n.setDouble(AREA, 0);
//        }

        // reset all areas to and add if column is available
        Iterator iter = new TreeNodeIterator(root);
        while ( iter.hasNext() ) {
            NodeItem n = (NodeItem)iter.next();
            n.setDouble(AREA, 0);
            int backingRow = n.getInt(sourceIndexField);
            Node backingNode = backingGraph.getNode(backingRow);
            if(n.getChildCount()==0){
                double a = backingNode.getDouble(backingAreaColumn);
                n.setDouble(AREA, a);
                n.setSize(n.getDouble(AREA));
            }
        }
