...
//    }
     ...
     * nodes past which nodes will not be visible nor expanded.
     */
    public FishEyeTreeFilter(String group, String sources, String focus, int distance) {
        super(group);
        m_focus = focus;
        m_sources = sources;
    ...
    public void run(double frac) {
        ...
        Iterator iter = m_vis.items(m_focus, m_groupP);
                //m_vis.getFocusGroup(Visualization.FOCUS_ITEMS).tuples();
        while (iter.hasNext()) {
...
//        iter = m_vis.getFocusGroup(m_focus).tuples();
//        while(iter.hasNext()){
//            visitFocus((NodeItem) iter.next(), null);
//            ((VisualItem)iter.next()).setDOI(1.0);
//        }
